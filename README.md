...
## Внеочередная административная проверка

В личном чате администратора отправьте `/start`: появится постоянная кнопка **🔎 Запустить внеочередную проверку**. Доступ проверяется по `TELEGRAM_ADMIN_CHAT_ID` и, если задан, `TELEGRAM_OWNER_ID`. Обычные пользователи меню не получают. Команда `/manual_review` открывает то же подтверждение.

До нажатия **✅ Да, подтверждаю** run не создаётся. **❌ Отмена** закрывает подтверждение. Подтверждение действует 15 минут и привязано к администратору; повторные нажатия, в том числе после завершения проверки, не создают второй run. Если проверка уже активна, бот отвечает «Я уже в работе» и присылает её результат после завершения. Нового run, pending manual job или дополнительного AI-запроса при этом нет.

Подтверждение создаёт `monitor_runs.trigger_type = manual-review` со статусом `running`. Дальше существующий минутный Supabase Cron продвигает полный pipeline по шагам; HTTP/webhook-запрос не держится открытым до окончания анализа. Для работы необходим активный `radar-worker` (включается через `npm run setup -- --activate`). Итог сохраняется в `run_reports` с обязательным review и приходит только в admin chat: индекс, предыдущий опубликованный индекс, динамика, изменения, findings, регионы, coverage, причины, независимые цепочки и источники. Длинный результат разбивается на сообщения; доставка повторяется без повторного AI-анализа. При неопределённом сетевом исходе приватное сообщение может повториться.

Manual run не создаёт публичный черновик или очередь публикации. Он исключён из агрегации ежедневного выпуска, а БД дополнительно запрещает создание `summaries` для его run ID. Настройки часов не меняются; пропущенные во время manual run плановые ключи остаются доступными, включая переход через полночь. Публичное форматирование не менялось; административные тексты находятся отдельно в `src/telegram/admin-text.ts`.

Для автоматизации доступен **`POST /api/manual-review`**, `Authorization: Bearer CRON_SECRET`. Сам авторизованный POST считается подтверждённым действием; тело не требуется. Необязательный заголовок `Idempotency-Key` должен быть UUID: используйте один и тот же UUID для повторов одного запроса. Без него каждый отдельный запрос после завершения предыдущего run может начать новую проверку.

Ответы: `{"ok":true,"status":"started","runId":"…"}` или `{"ok":true,"status":"already_running"}`. Повтор того же принятого ключа возвращает прежний run ID. `401` — нет доступа, `405` — неверный метод, `400` — неверный UUID; ошибки провайдеров и секреты не раскрываются. Ответы не кешируются.

Production-проверка после deployment: `/start` → кнопка → сначала отмена (run отсутствует), затем новый запрос → подтверждение. Повторное нажатие во время работы должно сообщить о текущей проверке; дождитесь результата в admin chat и убедитесь, что в канале нет внеочередной публикации. Для нового окружения сначала примените миграции `npm run db:migrate`; миграция manual review уже применена к текущему Supabase при реализации.
